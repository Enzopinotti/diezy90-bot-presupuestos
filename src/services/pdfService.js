...
// Cache para assets (logo, watermark)
const _assetCache = new Map();
async function readAsDataURICached(filePath) {
  if (_assetCache.has(filePath)) return _assetCache.get(filePath);
  const data = await readAsDataURI(filePath);
...
export async function generateBudgetHTML({ items = [], totals = {}, notFound = [], meta = {} } = {}) {
  ...
  const assetLogo = await readAsDataURICached(meta.assetLogoPath || B.assetLogoPath);
  const assetWatermark = await readAsDataURICached(meta.assetWatermarkPath || B.assetWatermarkPath);
...
export async function generateBudgetPDF({ items = [], totals = {}, notFound = [], meta = {} } = {}) {
  ...
  // Para HTML local con data URIs, solo necesitamos domcontentloaded (mucho más rápido)
